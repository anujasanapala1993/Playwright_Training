...
test('Salesforce: Login, Import File via Accounts, and Verify @smoke', async ({ page }) => {
  await page.goto('https://ability-ability-5739.my.salesforce.com');

  // 🔐 Login
  await page.fill('input#username', 'anuja.boddepalli-npcf@force.com');
  await page.fill('input#password', 'Anukrish@890');
...
// 🔼 Upload file
  const filePath = './tests/Test_Data/Salesforce_Accounts.csv'; // make sure SalesforceSample_accounts.csv exists
